...
const ProductCard: React.FC<OrderItemProps> = ({ orderItem }) => {
  ...
  function showDetails(orderItem: OrderItem) {
  ...
  return (
    <>
      <div className="max-w-sm rounded-2xl shadow-md bg-secondaryBackgroundColor p-4">
        ...
        <div
          onClick={() => showDetails(orderItem)}
          className="bg-hoverMenuActionsColor text-primaryTextColor  hover:bg-addToBasketHoverColor"
        >
          Beskrivelse
        </div>
      ...
      <Modal
        ...
        title={selectedOrderItem?.productname}
      >
        {selectedOrderItem && (
          <RichTextViewer html={selectedOrderItem.details} />
        )}
      </Modal>
