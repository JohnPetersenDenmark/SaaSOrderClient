...
import type { TruckLocation } from '../../core/types/TruckLocation';
...
const AdminCalendar: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        const locationsResponse : any= await get('/Home/truckcalendarlocationlist');
        const sortedTruckcalendarlocations = locationsResponse.sort((a : any, b): any => {
  ...
  const handleDelete = (truckLocation: TruckLocation) => {
    if (truckLocation !== null) {
      const deleteTruckLocation = async () => {
        try {
          setSubmitting(true);
          await remove('/Admin/removetrucklocation/' + truckLocation.id)
